...
interface DeleteRoomConfirmationModalProps extends ModalProps {
  index: number;
  onDelete: () => void;
}
...
export const DeleteRoomConfirmationModal = ({
  index,
  isOpen,
  onClose,
  onDelete,
}: DeleteRoomConfirmationModalProps): ReactElement => {
  return (
    <Modal isOpen={isOpen}>
      <IconWrapper size="lg" name="delete" isActive />
      <Text variant="heading" style={{ fontWeight: '700', marginVertical: 8, textAlign: 'center' }}>
        {`¿Deseas eliminar la habitación #${index + 1}?`}
      </Text>
      <Text variant="paragraph" style={{ textAlign: 'center', marginBottom: 32 }}>
        {`Al realizar esta acción se va eliminar habitación #${index + 1} de tu publicación`}
      </Text>
